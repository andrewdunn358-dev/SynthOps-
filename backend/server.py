...
@api_router.get("/reports/pdf/weekly-status")
async def export_weekly_status_pdf(user: dict = Depends(get_current_user)):
    """Export Weekly Status Report as PDF"""
    # Get report data
    now = datetime.now(timezone.utc)
    week_ago = (now - timedelta(days=7)).isoformat()
    
    tasks_resolved = await db.tasks.count_documents({
        "status": "completed",
        "updated_at": {"$gte": week_ago}
    })
    ...
    # Title
    story.append(Paragraph("Weekly Status Report", styles['ReportTitle']))
    story.append(Paragraph(f"Period: {week_ago[:10]} to {now.strftime('%Y-%m-%d')}", styles['ReportBody']))
    story.append(Spacer(1, 20))
    
    # Summary Table
    summary_data = [
        ['Metric', 'Value'],
        ['Tasks Completed', str(tasks_resolved)],
        ['Incidents Opened', str(len(incidents_week))],
        ['Incidents Resolved', str(len([i for i in incidents_week if i.get("status") == "resolved"]))],
        ['Total Servers', str(total_servers)],
        ['Servers Online', str(online_servers)],
        ['Uptime %', f"{round((online_servers / total_servers * 100) if total_servers > 0 else 100, 1)}%"],
        ['Hours Logged', f"{round(total_hours, 1)}h"],
    ]
    ...
    story.append(Paragraph(f"Generated by SynthOps on {now.strftime('%Y-%m-%d %H:%M')}", styles['ReportBody']))
    ...
    return Response(
        ...
        headers={"Content-Disposition": f"attachment; filename=weekly_status_{now.strftime('%Y%m%d')}.pdf"}
    )

@api_router.get("/reports/pdf/all-clients")
async def export_all_clients_pdf(user: dict = Depends(get_current_user)):
    """Export All Clients Summary as PDF"""
    clients = await db.clients.find({"is_active": True}, {"_id": 0}).to_list(1000)
    ...
    for c in clients:
        ...
        open_tasks = await db.tasks.count_documents({"client_id": c["id"], "status": {"$in": ["open", "in_progress"]}})
        uptime = round((online_count / server_count * 100) if server_count > 0 else 100, 1)
        health = min(100, max(0, 100 - open_incidents * 10 - (server_count - online_count) * 20))
    ...
    summaries.sort(key=lambda x: x["health"])
    ...
    now = datetime.now(timezone.utc)
    story.append(Paragraph("All Clients Health Summary", styles['ReportTitle']))
    story.append(Paragraph(f"Generated: {now.strftime('%Y-%m-%d %H:%M')}", styles['ReportBody']))
    story.append(Paragraph(f"Total Clients: {len(summaries)}", styles['ReportBody']))
    story.append(Spacer(1, 20))
    
    # Table
    table_data = [['Client', 'Servers', 'Online', 'Incidents', 'Tasks', 'Health']]
    for s in summaries[:50]:  # Limit to 50 clients for PDF
        table_data.append([
            s["name"][:30],
            str(s["servers"]),
            str(s["online"]),
    ...
    table = Table(table_data, colWidths=[2.5*inch, 0.7*inch, 0.7*inch, 0.9*inch, 0.7*inch, 0.7*inch])
    table.setStyle(TableStyle([
        ('BACKGROUND', (0, 0), (-1, 0), colors.HexColor('#1a1a2e')),
        ('TEXTCOLOR', (0, 0), (-1, 0), colors.whitesmoke),
    ...
    return Response(
        content=buffer.getvalue(),
        media_type="application/pdf",
        headers={"Content-Disposition": f"attachment; filename=clients_summary_{now.strftime('%Y%m%d')}.pdf"}
    )

@api_router.get("/reports/pdf/client/{client_id}")
async def export_client_report_pdf(client_id: str, user: dict = Depends(get_current_user)):
    """Export detailed Client Report as PDF"""
    client = await db.clients.find_one({"id": client_id}, {"_id": 0})
    if not client:
        raise HTTPException(status_code=404, detail="Client not found")
    
    servers = await db.servers.find({"client_id": client_id}, {"_id": 0}).to_list(1000)
    incidents = await db.incidents.find({"client_id": client_id}, {"_id": 0}).to_list(100)
    ...
    now = datetime.now(timezone.utc)
    story.append(Paragraph(f"Client Report: {client.get('name', 'Unknown')}", styles['ReportTitle']))
    story.append(Paragraph(f"Generated: {now.strftime('%Y-%m-%d %H:%M')}", styles['ReportBody']))
    story.append(Spacer(1, 20))
    ...
    summary_data = [
        ['Metric', 'Value'],
        ['Total Servers', str(len(servers))],
        ['Online', str(online)],
        ['Offline', str(len(servers) - online)],
        ['Uptime %', f"{round((online / len(servers) * 100) if servers else 100, 1)}%"],
    ...
    # Server List
    if servers:
        story.append(Paragraph("Servers", styles['SectionTitle']))
        server_data = [['Hostname', 'IP Address', 'OS', 'Status']]
        for s in servers[:30]:
            server_data.append([
                s.get("hostname", "")[:25],
                s.get("ip_address", ""),
                (s.get("operating_system", "") or "")[:20],
                s.get("status", "unknown")
            ])
        
        server_table = Table(server_data, colWidths=[2*inch, 1.3*inch, 1.7*inch, 0.8*inch])
    ...
    return Response(
        content=buffer.getvalue(),
        media_type="application/pdf",
        headers={"Content-Disposition": f"attachment; filename=client_{client.get('code', client_id)}_{now.strftime('%Y%m%d')}.pdf"}
    )

@api_router.get("/reports/pdf/incident-trends")
async def export_incident_trends_pdf(days: int = 90, user: dict = Depends(get_current_user)):
    """Export Incident Trends Report as PDF"""
    cutoff = (datetime.now(timezone.utc) - timedelta(days=days)).isoformat()
    incidents = await db.incidents.find({"date_opened": {"$gte": cutoff}}, {"_id": 0}).to_list(1000)
    ...
    now = datetime.now(timezone.utc)
    story.append(Paragraph(f"Incident Trends Report ({days} Days)", styles['ReportTitle']))
    story.append(Paragraph(f"Generated: {now.strftime('%Y-%m-%d %H:%M')}", styles['ReportBody']))
    story.append(Paragraph(f"Total Incidents: {len(incidents)}", styles['ReportBody']))
    story.append(Spacer(1, 20))
    
    # By Severity
    story.append(Paragraph("Incidents by Severity", styles['SectionTitle']))
    sev_data = [['Severity', 'Count', 'Percentage']]
    total = len(incidents) or 1
    for sev, count in by_severity.items():
        sev_data.append([sev.capitalize(), str(count), f"{round(count/total*100, 1)}%"])
    ...
    # Top Clients
    story.append(Paragraph("Top 10 Clients by Incidents", styles['SectionTitle']))
    client_data = [['Client', 'Incidents']]
    for client, count in sorted(by_client.items(), key=lambda x: -x[1])[:10]:
        client_data.append([client[:35], str(count)])
    
    client_table = Table(client_data, colWidths=[4*inch, 1.5*inch])
    ...
    return Response(
        content=buffer.getvalue(),
        media_type="application/pdf",
        headers={"Content-Disposition": f"attachment; filename=incident_trends_{now.strftime('%Y%m%d')}.pdf"}
    )


# ==================== HISTORICAL TREND DATA ====================

@api_router.get("/reports/trends/incidents")
async def get_incident_trend_data(days: int = 90, user: dict = Depends(get_current_user)):
    """Get incident data for trend charts"""
    cutoff = (datetime.now(timezone.utc) - timedelta(days=days)).isoformat()
    incidents = await db.incidents.find({"date_opened": {"$gte": cutoff}}, {"_id": 0}).to_list(2000)
    ...
    for i in incidents:
        ...
        if date:
            if date not in by_date:
                by_date[date] = {"total": 0, "critical": 0, "high": 0, "medium": 0, "low": 0}
    ...
    while current <= end:
        ...
        data = by_date.get(date_str, {"total": 0, "critical": 0, "high": 0, "medium": 0, "low": 0})
        chart_data.append({
            "date": date_str,
            "total": data["total"],
...
@api_router.get("/reports/trends/tasks")
async def get_task_trend_data(days: int = 90, user: dict = Depends(get_current_user)):
    """Get task completion data for trend charts"""
    cutoff = (datetime.now(timezone.utc) - timedelta(days=days)).isoformat()
    tasks = await db.tasks.find({
        "updated_at": {"$gte": cutoff}
    }, {"_id": 0, "status": 1, "created_at": 1, "updated_at": 1}).to_list(5000)
    ...
    for t in tasks:
        ...
        if date:
            if date not in by_date:
                by_date[date] = {"created": 0, "completed": 0}
            if t.get("status") == "completed":
                by_date[date]["completed"] += 1
            else:
                by_date[date]["created"] += 1
    ...
    while current <= end:
        date_str = current.strftime("%Y-%m-%d")
        data = by_date.get(date_str, {"created": 0, "completed": 0})
        chart_data.append({
            "date": date_str,
...
@api_router.get("/reports/trends/time-logged")
async def get_time_logged_trend_data(days: int = 30, user: dict = Depends(get_current_user)):
    """Get time logged data for trend charts"""
    cutoff = (datetime.now(timezone.utc) - timedelta(days=days)).strftime("%Y-%m-%d")
    entries = await db.time_entries.find({
        "entry_date": {"$gte": cutoff}
    }, {"_id": 0, "entry_date": 1, "duration_minutes": 1}).to_list(10000)
    
    by_date = {}
    for e in entries:
        date = e.get("entry_date", "")[:10]
        if date:
            by_date[date] = by_date.get(date, 0) + e.get("duration_minutes", 0)
    ...
    while current <= end:
        date_str = current.strftime("%Y-%m-%d")
        minutes = by_date.get(date_str, 0)
        chart_data.append({
            "date": date_str,
        ...
        if device_type == "ping":
            ...
            if token_id and token_secret:
                ...
                async with httpx.AsyncClient(verify=False, timeout=10.0) as client:
                    url = f"https://{ip_address}:{port or 8006}/api2/json/version"
                    ...
                    if resp.status_code == 200:
                        ...
                        nodes_resp = await client.get(
                            f"https://{ip_address}:{port or 8006}/api2/json/nodes",
                            headers=headers
                        )
                        if nodes_resp.status_code == 200:
                            result["extra_data"]["nodes"] = nodes_resp.json().get("data", [])
            else:
