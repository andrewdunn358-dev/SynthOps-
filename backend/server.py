...
async def list_customers(user: dict = Depends(get_current_user)):
    ...
    for c in customers:
        ...
        result.append(CustomerResponse(
            ...
            contract_start=c["contract_start"] if isinstance(c.get("contract_start"), datetime) else (datetime.fromisoformat(c["contract_start"]) if c.get("contract_start") else None),
            contract_end=c["contract_end"] if isinstance(c.get("contract_end"), datetime) else (datetime.fromisoformat(c["contract_end"]) if c.get("contract_end") else None),
            ...
            tags=c.get("tags", []),
            is_active=c.get("is_active", True),
            created_at=c["created_at"] if isinstance(c.get("created_at"), datetime) else datetime.fromisoformat(c["created_at"]),
            updated_at=c["updated_at"] if isinstance(c.get("updated_at"), datetime) else (datetime.fromisoformat(c["updated_at"]) if c.get("updated_at") else None),
            notes_count=notes_count,
